...
- `storyboard-draft@1.4.0` Plugin Contract 已形成服务端只读白名单，固定 Operation、Skill Route、输入输出 Role、工具、超时和重试上限。
- NodeRun 已支持持久化排队、Runner 租约领取、SSE 进度、取消、超时、一次重试和重启后恢复排队任务。
- Local Contract Runner 已完成注册、心跳与运行状态查询；制作端和管理端均读取真实服务状态。
- NodeRun 完成后生成 Receipt，记录实际 Skill Route、Runner、输入 Artifact 快照、Validator 结果和输出 Artifact ID。
- API 明确拒绝浏览器提交 `command`、`shell` 或 `cwd`；只有管理员或系统执行器能创建、取消和重试 NodeRun。
- 当前执行适配器为 `contract-dry-run`，只验证安全控制面，不伪造 Codex 图片产物；收据使用 `NEEDS_CODEX_ADAPTER` 明示边界。
- 配置 `CODEX_GATEWAY_URL` 后，工作台通过现有 Remote Codex Control Gateway 创建任务，并持久化 `nodeRunId ↔ taskId ↔ threadId ↔ turnId`。
- Remote Runner 已支持把固定 `workspace-write / approvalPolicy=never / ephemeral=false` 传入 `thread/start`；浏览器无权修改这些参数。
- 每个 attempt 创建独立运行目录，输入 Artifact 复制为只读快照，唯一允许写入的业务目录为 `staging/`。
- Collector 只接受合法 `node-result.json`；校验普通文件、真实路径、符号链接边界、Role、镜号覆盖和 SHA-256 后，原子提交为 `committed-outputs`。
- 输出通过后登记真实 Artifact，并将 Hash、Codex Binding、Validator 结果和 committed root 写入 NodeRun Receipt。
- 前后端 Preflight 要求修复运行至少具备正式脚本、视觉参考以及 S12/S14 相邻镜；缺失输入返回 `NODE_RUN_PREFLIGHT_FAILED`，不会创建 Codex Turn。
- 真实烟测已验证 NodeRun 成功绑定 Gateway task、Codex thread/turn，且取消请求能中断 turn 并回写 `CANCELLED`。
- 镜号覆盖 Evaluator。
...
- EventSource 已支持刷新恢复、心跳与多标签页执行锁；服务端持久化租约已作为真正执行所有权边界。
...
- Model Judge。
- 正式身份认证、工程级授权和角色管理。
- 真实图片生成所需的完整 EP 输入绑定与 Skill Route Registry 哈希验证。
- Plugin Contract 的多版本注册、审批与发布界面。
- 生产级用户身份和工程权限。
...
- 已完成：NodeRun 持久化队列、Runner 注册和心跳。
- 已完成：已发布 Plugin Contract 解析和 Skill Route 冻结。
- 已完成：取消、超时、一次重试、SSE 状态、审计和 NodeRun Receipt。
- 已完成：拒绝来自浏览器的任意命令、Shell 和工作目录参数。
- 已完成：连接 Remote Codex Control Gateway，并绑定真实 Codex task/thread/turn。
- 已完成：每次运行的隔离目录、只读输入、固定沙盒与 Artifact 输出登记。
- 已完成：`node-result.json`、路径、符号链接、Role、镜号与 Hash Collector。
- 待完成：Skill Route Registry 的版本与哈希冻结，以及使用完整 EP 资产执行首张真实 S13。
...
- 代码自动测试：35/35 通过。
- 正式前端构建：通过。
- API 测试覆盖：幂等、持久化、SSE、断线续传、Artifact Hash、来源链、Gate、NodeRun 取消/超时/重试和 Receipt。
...
```text
Skill Route Registry 哈希冻结 + 完整 EP 输入绑定
  → 正式身份与工程授权
    → Vision / Model Judge
```
